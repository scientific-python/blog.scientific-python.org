...
Going all the way back to my post on 24 May 2021 titled [*Networkx Function stubs*](https://blog.scientific-python.org/posts/networkx/networkX-function-stubs.html) the only function left is `asadpour_tsp`, the main function which needs to accomplish this entire algorithm. 
...
Recall from my post [*Networkx Function Stubs*](https://blog.scientific-python.org/posts/networkx/networkX-function-stubs.html) what the primary traveling salesman function, `traveling_salesman_problem` will ensure that we are given a complete graph that follows the triangle inequality by using all-pairs shortest path calculations and will handle if we are expected to return a true cycle or only a path.
