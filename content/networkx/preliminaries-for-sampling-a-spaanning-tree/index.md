...
> Given \\(\lambda\_e \geq 0\\) for \\(e \in E\\), a \\(\lambda\\)*-random tree* \\(T\\) of \\(G\\) is a tree \\(T\\) chosen from the set of all spanning trees of \\(G\\) with probability proportional to \\(\prod\_{e \in T} \lambda\_e\\).
